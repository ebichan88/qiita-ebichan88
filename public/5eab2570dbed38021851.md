---
title: JavaエンジニアがTypeScriptに触れて違いを感じた7つのポイント
tags:
  - Java
  - JavaScript
  - TypeScript
private: false
updated_at: '2026-01-12T14:05:34+09:00'
...
```diff_typescript:TypeScript：typeの例
-interface Character {
-  attack(): void;
-}
+type Character = {
...
例外処理について、TypeScriptでは`try-catch-finally`構文が存在し、これはJavaと同様です。
...
Javaには**検査例外**と呼ばれる、メソッドのシグネチャに`throws`宣言が必要な例外があります。
...
実装段階で例外処理を考慮した実装を強制するのがJavaの特徴ですが、TypeScriptには検査例外や`throws`宣言が存在しません。
...
TypeScript / JavaScriptでは、例外をThrowする場合、`Error`オブジェクトを利用するのが一般的です。

ただし、`Error`以外にも任意の型の値を`throw`することが可能です。
先ほどのコードを下記のように書き換えることもできます。

```diff_typescript:TypeScript
function divide(a: number, b: number): number {
  if (b === 0) {
-    throw new Error("Division by zero is not allowed");
...
[^3]: [サバイバルTypeScript 例外処理 (exception)](https://typescriptbook.jp/reference/statements/exception#throw構文)

### catch 変数は unknown（型安全）

Javaでは、`catch`ブロックで受け取る例外変数の型はデフォルトで`Throwable`型となります。
Throwされる例外オブジェクト毎に処理を分岐したい場合は、複数の`catch`ブロックを用意します。
...
一方TypeScriptでは、先ほど述べた通りどんな型の値でも`throw`できるため、`catch`ブロックで受け取る例外変数の型はデフォルトで`unknown`型となります。

`unknown`型の場合、そのままではプロパティにアクセスできないため、型ガードを用いて型を絞り込む必要があります。
...
Javaのように例外オブジェクト毎に処理を分岐したい場合、`catch`を複数用意するのではなく、`catch`内で分岐処理を実装します。
...
TypeScriptは`undefined`や例外処理など、JavaScriptでは実行時エラーとなりそうな部分をコンパイル時にチェックしてくれるのは非常にありがたいと感じました。
